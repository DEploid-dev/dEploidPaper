...
\subsection{Deconvoluting examples}
\paragraph{Deconvoluting sample {\textmd PG0396-C}}
...
\centering
\subfloat[][]{
\includegraphics[width=.47\textwidth]{{supplementDEploidExample/PG0396-C_seed5k3.interpretDEploidFigure.1}.pdf}
}%\label{fig:pg0396.interpret1}
\subfloat[][]{
\includegraphics[width=.53\textwidth]{{supplementDEploidExample/PG0396-C_seed5k3.interpretDEploidFigure.2}.pdf}
}\\
%\caption{}
%\end{figure}
...
%\ContinuedFloat
\subfloat[][]{
\includegraphics[width=.32\textwidth]{{supplementDEploidExample/PG0396-C_seed5k3.single0}.pdf}
}
\subfloat[][]{
\includegraphics[width=.32\textwidth]{{supplementDEploidExample/PG0396-C_seed5k3.single1}.pdf}
}
\subfloat[][]{
\includegraphics[width=.32\textwidth]{{supplementDEploidExample/PG0396-C_seed5k3.single2}.pdf}
}\\
...
\paragraph{Deconvoluting sample {\textmd PG0415-C}}
...
\begin{figure}[th]
\subfloat[][]{
\includegraphics[width=.47\textwidth]{{supplementDEploidExample/PG0415-C_seed2k3.interpretDEploidFigure.1}.pdf}
}
\subfloat[][]{
\includegraphics[width=.53\textwidth]{{supplementDEploidExample/PG0415-C_seed2k3.interpretDEploidFigure.2}.pdf}
}\\
\subfloat[][]{
\includegraphics[width=.32\textwidth]{{supplementDEploidExample/PG0415-C_seed2k3.single0}.pdf}
}\label{fig:pg0415.postprob_a}
\subfloat[][]{
\includegraphics[width=.32\textwidth]{{supplementDEploidExample/PG0415-C_seed2k3.single1}.pdf}
}\label{fig:pg0415.postprob_b}
\subfloat[][]{
\includegraphics[width=.32\textwidth]{{supplementDEploidExample/PG0415-C_seed2k3.single2}.pdf}
}\label{fig:pg0415.postprob_c}\\
\caption{Sample {\textmd PG0415-C} deconvolution with Reference Panel V. (a) Diagnostic panels from the \texttt{DEploid} output. The top three panels recap the data exploration process, with an enhanced PLAF vs WASF plot: red dots show observed WSAF, which is calculated by read counts; blue does show the expected WSAF inferred from our model. The next three plots from left to right show: 1. MCMC samples for the strain proportions. 2. Expected WSAF vs observed WSAF. We use the correlation between the observed and expected WSAF as a sanity check for our model. 3. Log likelihood of the MCMC chain. The colored dots mark the likelihoods of the model when specific MCMC steps are used: updating the strain proportions, painting a single haplotype and painting a pair of haplotypes are marked in green red and blue respectively. (b) Expected WSAF (blue) and observed WSAF (red) across the genome. This figure highlights the genome diversity within the mixed sample across the genome. (c) (d) and (e) show the posterior painting probabilities for the deconvoluted strains when using the reference panel V. In each figure, 3D7, HB3, 7G8 and Dd2 are represented by colors red, light orange, yellow and dark orange respectively. Each panel represents the posterior probability of a chromosome. Chromosomes 1 -- 14 are ordered from left to right, then top to bottom.}\label{fig:0415}
\end{figure}
...
\paragraph{Deconvoluting field sample {\textmd PD0577-C}}
As part of the Pf3k project analysis, we have applied DEploid to all 2512 field samples. Figures~\ref{fig:PD0577} (a)--(e) show example of deconvoluting mixed sample {\textmd PD0577-C} with calculated inbreeding probabilities within sample.

\begin{figure}[ht]
\centering
\subfloat[][%Diagnostic panels from the DEploid output.
]{
\includegraphics[width=.47\textwidth]{{pf3kSimField/PD0577-C/PD0577-C.eg.inbreeding.interpretDEploidFigure.1}.pdf}
}%\label{fig:pg0396.interpret1}
\subfloat[][%Allele frequencies within sample across the genome.
]{
\includegraphics[width=.53\textwidth]{{pf3kSimField/PD0577-C/PD0577-C.eg.inbreeding.interpretDEploidFigure.2}.pdf}
}\\

\subfloat[][%Posterior painting probabilities for\\the deconvoluted strain 1 when using \\the reference panel Asia-1.
]{
\includegraphics[width=.32\textwidth]{{pf3kSimField/PD0577-C/PD0577-C.eg.inbreeding.single0.inbreeding}.pdf}
}
\subfloat[][%Posterior painting probabilities for\\the deconvoluted strain 2 when using \\the reference panel Asia-1.
]{
\includegraphics[width=.32\textwidth]{{pf3kSimField/PD0577-C/PD0577-C.eg.inbreeding.single1.inbreeding}.pdf}
}
\subfloat[][%Posterior painting probabilities for\\the deconvoluted strain 3 when using the\\reference panel Asia-1.
]{
\includegraphics[width=.32\textwidth]{{pf3kSimField/PD0577-C/PD0577-C.eg.inbreeding.single2.inbreeding}.pdf}
}\\
\caption{Sample {\textmd PD0577-C} deconvolution with 10 reference strains from Asia. (a) Diagnostic panels from the \texttt{DEploid} output. The top three panels recap the data exploration process, with an enhanced PLAF vs WASF plot: red dots show observed WSAF, which is calculated by read counts; blue does show the expected WSAF inferred from our model. The next three plots from left to right show: 1. MCMC samples for the strain proportions. 2. Expected WSAF vs observed WSAF. We use the correlation between the observed and expected WSAF as a sanity check for our model. 3. Log likelihood of the MCMC chain. The colored dots mark the likelihoods of the model when specific MCMC steps are used: updating the strain proportions, painting a single haplotype and painting a pair of haplotypes are marked in green red and blue respectively. (b) Expected WSAF (blue) and observed WSAF (red) across the genome. This figure highlights the genome diversity within the mixed sample across the genome. (c) (d) and (e) show the inbreeding probabilities (red) for the deconvoluted strains. Each panel represents the inbreeding probability of a chromosome. Chromosomes 1 -- 14 are ordered from left to right, then top to bottom.
}\label{fig:PD0577}
\end{figure}
