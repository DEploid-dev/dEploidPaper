...
A mixed infection can be completely described by the number of co-existing strains, their relative proportions, and their associated haplotypes. Existing methods for characterizing mixed infections are limited to providing a summary statistic of relative inbreeding (Fws, \cite{Manske2012}), inferring the number of strains (\texttt{COIL}), or simultaneously inferring the number of strains and their proportions (\texttt{pfmix}, \cite{Jack2016}). DEploid is the only method that can also estimate haplotypes although it can be argued that conventional tools for phasing diploid organisms (\texttt{BEAGLE}, \texttt{SHAPEIT}) could be used to deconvolute mixtures of two strains.

In this section, we use the same dataset (27 samples) to compare \texttt{DEploid} with all the inferential methods mentioned above (see supplementary material for details). Our method shows robust inference on the number of strains when relative proportions are above 1\%. DEploid correctly infers the number of strains in 26 out of 27 samples. In comparison, \texttt{COIL} correctly infers the number of strains in 23 samples. We notice that \texttt{COIL} struggles to identify strains whose relative proportions is below 5\% (Figure~\ref{fig:switchVsMisCopy}(a)). Regarding the inference of relative proportions, \texttt{DEploid} and \texttt{pfmix} produce similar results with minor differences (Figure~\ref{fig:switchVsMisCopy}(b)), with pfmix deviating from the truth at most 3.6\%, and \texttt{DEploid} exhibiting a maximum error of 2\%.

We also experimented with \texttt{BEAGLE} and \texttt{SHAPEIT} for deconvoluting haplotypes in mixtures of two strains. Both methods worked well for balanced mixtures (i.e. with proportions between 40\% and 60\%) as they mimic a diploid sample. However, as strain proportions became more unbalanced, accuracy degraded and both methods wrongly inferred heterozygous sites as homozygous, introducing a bias towards inferring the haplotypes of dominant strains. We observed that strains with a relative proportion below 20\% were always masked out by the dominant strain (Figure~\ref{fig:switchVsMisCopy}(c)).



%The number of strains, strain proportions and haplotypes are three key components used to describe mixed infections. Existing methods could only infer one of the three variables; or, at the best, simultaneously estimate the first two variables. To systematically resolve all three variables has only become available until our method.

%In order to show how our method can significantly advance studies in relation to mixed infections, we have compared our inference results on number of strains, strain proportions and haplotypes with COIL, pfmix\citep{Jack2016}, BEAGLE and SHAPEIT, using the same dataset (see supplementary material for details).

%Our method shows robust inference result on number of strains when relative abundance of each strain is above 1\%. DEploid correctly infers the number of strain for 26 samples out of 27. In comparison, COIL infers correctly three cases less than DEploid: COIL struggles to identify strain of which proportion is below 5\% (Figure~\ref{fig:switchVsMisCopy}(a)).

%The proportion inference of both \citet{Jack2016}'s and our methods are consistent with minor differences. The deviation between the estimates from pfmix and the truth (see {Figure~\ref{fig:switchVsMisCopy}(b)}) is within 3.6\%; whereas DEploid estimates differ 2\%  at the most.

%We experimented to haplotype phasing with conventional tools for diploid samples: BEAGLE and SHAPEIT. As expected, these methods only returned meaningful results for mixture of two strains. Both BEAGLE and SHAPEIT worked well for balanced mixtures (proportions between 40\% and 60\%) in the same fashion as a diploid sample. However, as the result of tilted proportions, both BEAGLE and SHAPEIT wrongly inferred heterozygous sites as homozygous, which bias towards dominant strains. Strains with proportions below 20\% were inferred the same as the dominant strains (Figure~\ref{fig:switchVsMisCopy}(c)).


\subsection{Run-time}

The complexity of our program is $\mathcal{O}(lm^2)$ (see Figure~\ref{fig:runtime}), where $m$ and $l$ are the number of reference strains and sites respectively. In practice, we recommend dividing samples into distinct geographical regions to perform deconvolution, using the ten most different local clonal strains as as reference panel. The run time of deconvoluting a field sample range between 1 and 6 hours, depending on the number variants in a sample: For example, it takes $5\frac{1}{2}$ hours to process sample {\textmd QG0182-C} over 372,884 sites.  We give worked examples of deconvoluting mixed infections from {\it in vitro} samples in the Supplementary Material.
