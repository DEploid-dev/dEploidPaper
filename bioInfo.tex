...
Note that this data set only contains two clonal-ish samples, which are not ideal for constructing a reference panel due to the limited size. Alternatively, we have experimented with the following different reference panels; in all cases we estimated the number and proportion of strains accurately: Figure~\ref{fig:differentRefPanel} y-axises show the proportions of strains Dd2/7G8/HB3 as approximately 25/50/25\%.
\begin{itemize}
...
To measure the accuracy of the inferred haplotypes, each inferred haplotype is then divided into segments with length of 50, then mapped onto panel V reference strains according to the indices.
