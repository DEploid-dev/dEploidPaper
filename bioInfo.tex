...
Let $\mathbf w = [w_1,\dots, w_n]$ and $\mathbf{h}_i = [h_{1,i},\dots,h_{n,i}]$ denote the proportions and alleic states of the $n$ parasite strains at site $i$. We use \citet{Jack2016}\todo{RE: reviewer 1, O'Brien (2016)}'s expression for the expected WSAF at site $i$, $q_{i}$, as:
...
\noindent We model over-dispersion in read counts relative to the Binomial using a Beta-binomial distribution. Specifically, the read counts of `ALT' are identically and independently distributed (i.i.d.) Bernoulli random variables with probability of success $v_i$; i.e. $a_i \sim Binom(a_i + r_i, v_i)$, and $v_i \sim Beta(\alpha, \beta)$, where $E(v_i) = \alpha/(\alpha+\beta) = \pi_{i}$. This is achieved by setting $\alpha = c\cdot \pi_{i} $ and $\beta = c\cdot (1-\pi_{i})$, such that the variance of the WSAF is \textcolor{red}{inversely} \todo{RE: reviewer 3, sp inversley} proportion to $c$. \todo[inline]{TODO, RE: reviewer 3, " what is c? it isn't defined? In general the model section needs some effort in clarification."} Combined, we have:
...
\label{tab:jason}
\todo[inline]{TODO RE: reviewer 2: the percentages shown could vary when re-run with the same parameters. Instead of single values, distributions (e.g. means and variances) need to be shown.}
\end{table}
...
\todo[inline]{RE: reviewer 3: black bars was confusing}
\todo[inline]{TODO RE: reviewer 3:  I was not clear what the general takehome message was meant to be}
\end{figure*}
...
\todo[inline]{RE: reviewer 3, "c is a noisy plot. It would be much clearer if shown with a smoothing."}
\todo[inline]{TODO RE: reviewer 3,  It would inform the reader to say what the take home message of all plots should be in the legend.}
\todo[inline]{TODO RE: reviewer 1, cannot read the figure legends and axes.}
\end{figure*}
...
\todo[inline]{TODO RE: reviewer 2, Application to other species (Discussion).}

\todo[inline]{TODO RE: reviewer 2:
...
\todo[inline]{TODO RE: reviewer 2, InDels and structural variants. When reconstructing haplotypes, indels and structural variation also need to be considered, while dEploid only reconstructs SNPs. This should be address in the discussion. }
