...
In order to assess the accuracy of the inferred haplotypes, we want to take into account of both swithch errors () and genotype miss copying (discordance between ). Since we have used , however this is difficult.

overestimates switch errors

unable to align the flips

Alternatively,


each inferred haplotype is then divided into segments with length of 50, then mapped onto panel V reference strains according to the indices.

In order to test how sensitive our model with coverage, data is simulated by sampling read counts according to a $Bin(n,p)$ model, where $n$ is the number alternative and reference alleles at each site. Four different probabilities $p$: 0.2, 0.5, 0.8 and 1.0 are used for simulating the senariors of lower, median and high coverage data and control. We have found that

, but the accuracy of the haplotype is dependent on having an approriate reference panel .


the proportion affect the inference results as well, see Fig~\ref{fig:swithVsMisCopy}


the affect of coverage ...
...
\caption{We use panel V to deconvolute all 27 samples.  }\label{fig:swithVsMisCopy}
\end{figure}
