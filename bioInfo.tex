...
\subtitle{\textcolor{red}{???Genetics and population analysis}\\ \textcolor{red}{???Genome analysis}}
...
\end{equation}
Thus, the likelihood of $L(\mathbf{w}, \mathbf{h},\Xi, e|D)$ in Eqn.~\eqref{eqn:post} becomes $L(q_{i},\Xi, e|D)$. % In the rest of the paper, we will use the expressions $P(D|\theta)$ and $L(\theta|D)$ interchangeably for convience, where $\theta$ represents a vector of parameters, in this case $[\mathbf{w}, \mathbf{h},\Xi, e]$.
...
% $\exp(x_k)$ (normailized by the sum of $\exp(x_k)$). %We consider the prior on $\mathbf{w}|n$ as the prior on $\mathbf{x}|n$. Suppose the probability density of $x_k$ is $\phi(x|\eta, \sigma^2)$, the prior density on each $w_k$ then becomes $\phi(\log(w_k)|\eta, \sigma^2)\frac{d}{dw_k}\log(w_k)$.
