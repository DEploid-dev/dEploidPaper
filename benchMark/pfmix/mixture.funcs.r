run.mcmc = function(num.test,allele.freq,data.set,num.iter,thin, nrepeat = 10)
{
	...
	for(j in 1:nrepeat)
	{
		model.mcmc[[j]] = mh.mcmc(num.test, allele.freq, data.set, num.iter,thin)
	}
	...
	for (j in 1:nrepeat)
	{
		...
		elt = which.min(bic);
#                print(model.mcmc[[j]][[1]][[1]]$p)
                cat( sort(model.mcmc[[j]][[1]][[1]]$p[2:(2+model.mcmc[[j]][[1]][[1]]$num.comp)])[1:model.mcmc[[j]][[1]][[1]]$num.comp], " ")
                cat("bic = ",bic[j], "\n")
	}
...
init.mixture.model = function(num.comp,allele.freq, alpha=0.1, nu=25,p=NA,val=1)
{
	if (is.na(p[1]))
	{	p = generate.p(num.comp)	}
...
{
	current  = init.mixture.model(num.comp,allele.freq)
	curr.llk = calc.llk(data.set, model = current, allele.freq = allele.freq)
